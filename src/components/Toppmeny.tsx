...
export default function Toppmeny() {
  ...
  const [showMobilMeny, setShowMobilMeny] = useState(false)
  ...
  const mobilMenyRef = useRef<HTMLDivElement>(null)
  ...
  useEffect(() => {
    function handleClickOutside(e: MouseEvent) {
      ...
      if (mobilMenyRef.current && !mobilMenyRef.current.contains(e.target as Node)) {
        setShowMobilMeny(false)
      }
    }
  ...
  return (
    <header className="bg-[#0F1923] text-white sticky top-0 z-50">
        ...
        <div className="flex items-center justify-between h-14">
          ...
          <div className="flex items-center gap-8">
            <div className="flex items-center gap-3">
              <button
                onClick={() => setShowMobilMeny(!showMobilMeny)}
                className="sm:hidden p-1.5 rounded-md text-gray-300 hover:text-white hover:bg-white/10 transition-colors"
                aria-label="Meny"
              >
                {showMobilMeny ? (
      ...
      {/* Mobilmeny */}
      {showMobilMeny && (
        <div ref={mobilMenyRef} className="sm:hidden border-t border-white/10">
